/**
 * V1 rail wallet: Wallet Standard over Solana mainnet-beta.
 *
 * Mounted unconditionally alongside the EVM provider — see wallet-context.tsx,
 * which picks whichever one the active rail needs. Detection is cheap and both
 * stacks have to be live for the rail switch to be instant.
 */
import {
...
export interface SolanaWalletValue {
...
const SolanaWalletContext = createContext<SolanaWalletValue | null>(null);
...
export function SolanaWalletProvider({ children }: { children: ReactNode }) {
  ...
  const value = useMemo<SolanaWalletValue>(
    () => ({
      wallets,
      account,
...
export function useSolanaWallet(): SolanaWalletValue {
